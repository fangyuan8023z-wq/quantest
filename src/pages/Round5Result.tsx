...
import { round5Questions, getRound5Score } from "../data/round5";

const Round5Result = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const state = location.state as any;
  const answers = state?.round5Answers || [];
  const score = getRound5Score(answers);
    ...
    >
      <div className="max-w-[560px] mx-auto space-y-5">
        <div className="bg-white rounded-[20px] shadow p-[40px] text-center">
          ...
          <p className="text-[15px]" style={{ color: "#6b8f7a" }}>
            {correct}/{round5Questions.length} 题正确（每题2分）
          </p>
        </div>
