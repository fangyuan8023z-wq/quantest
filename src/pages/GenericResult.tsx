...
const GenericResult = ({
  ...
  title: string;
  questions: Question[];
  getScore: (answers: any[]) => number;
  nextRoute: string;
  prevData?: any;
}) => {
  const location = useLocation();
  const navigate = useNavigate();
  const state = (location.state as any) || propData || {};

  const answersKey =
    Object.keys(state).find((k) => k.includes("Answers")) || `${title}Answers`;
  const currentAnswers = state[answersKey] || [];

  const score = getScore(currentAnswers);
      ...
      <div className="max-w-[560px] mx-auto space-y-5">
        ...
        <div className="bg-white rounded-[20px] shadow p-[24px]">
          ...
          <div className="space-y-3">
            {questions.map((q) => {
              const ua = currentAnswers.find((a: any) => a.qId === q.id);
              const ca = Array.isArray(q.answer) ? q.answer : [q.answer];
